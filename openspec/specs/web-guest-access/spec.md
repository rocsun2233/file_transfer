...
The system SHALL expose a LAN-reachable web interface on each running device so that another device can access limited file transfer functions through a browser without installing the desktop client.
...
#### Scenario: Android browser opens desktop-hosted LAN address
- **WHEN** a desktop client is running on the LAN and an Android browser opens the desktop-published web address
- **THEN** the system accepts the browser connection on the desktop host's LAN interface instead of limiting access to localhost only
