...
### Requirement: Desktop application shows LAN access endpoints
The desktop application SHALL show the currently published LAN access information for the running device, including the browser URL and transfer endpoint that peers can use to connect.

#### Scenario: User checks the current device access address
- **WHEN** the desktop application is running
- **THEN** the desktop application shows the active bind host plus one or more LAN-reachable browser URLs and transfer endpoints for the current device
